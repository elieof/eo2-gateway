package com.fahkap.eo2.gateway.web.rest;

import com.fahkap.eo2.gateway.config.KafkaProperties;
...
class Eo2GatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        Eo2GatewayKafkaResource kafkaResource = new Eo2GatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/eo-2-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/eo-2-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
