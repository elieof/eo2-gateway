...
describe('QResult e2e test', () => {
  ...
  it('should load QResults', async () => {
    ...
    expect(await qResultComponentsPage.getTitle()).to.eq('eo2GatewayApp.quizQResult.home.title');
  ...
  it('should load create QResult page', async () => {
    ...
    expect(await qResultUpdatePage.getPageTitle()).to.eq('eo2GatewayApp.quizQResult.home.createOrEditLabel');
  ...
  it('should create and save QResults', async () => {
    ...
    await promise.all([qResultUpdatePage.setUsernameInput('username'), qResultUpdatePage.questionSelectLastOption()]);

    expect(await qResultUpdatePage.getUsernameInput()).to.eq('username', 'Expected Username value to be equals to username');
  ...
  it('should delete last QResult', async () => {
    ...
    expect(await qResultDeleteDialog.getDialogTitle()).to.eq('eo2GatewayApp.quizQResult.delete.question');
